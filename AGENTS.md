...
- 一个知识领域内部形成多个稳定、可持续扩展的子主题时，必须建立独立子目录，不得把所有笔记长期平铺在领域根目录。例如 Python 下的数据结构与算法（Data Structures and Algorithms）应放入独立的 `数据结构与算法（Data Structures and Algorithms）/` 子目录。
- 子目录必须按真实知识边界划分，不能按来源机构、课程名称或临时批次命名；适合独立成目录的主题应至少具有多篇现有笔记，或已有明确的连续扩展计划。
