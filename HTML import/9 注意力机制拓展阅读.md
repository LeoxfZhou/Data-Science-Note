...
- [[#1|1 注意力机制原理]]
	...
	- [[#11|1.1 注意力机制示意图]]
	- [[#12-attention|1.2 Attention计算过程]]
	- [[#13-attention|1.3 Attention计算逻辑]]
	- [[#14-attention|1.4 有无attention模型对比]]
        
		- [[#1-attention|1 无attention机制的模型]]
		- [[#2-attention|2 有attention机制的模型]]
...
### 1.2 Attention计算过程[[#12-attention|¶]]
...
- 这里为了生成单词，我们把 Decoder 部分输入后得到的向量作为 query；把 Encoder 部分每个单词的向量作为 key。首先我们先把 query 和 每一个单词进行点乘score=query\cdot key，得到相关性的分值；
...
### 1.3 Attention计算逻辑[[#13-attention|¶]]
...
### 1.4 有无attention模型对比[[#14-attention|¶]]

#### 1 无attention机制的模型[[#1-attention|¶]]
