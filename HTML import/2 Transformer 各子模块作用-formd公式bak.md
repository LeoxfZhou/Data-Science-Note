...
![[image-20220615101108585.png]]

MultiHead(Q,K,V)=Concat(head_1,\cdots , head_h)W^O\\\\ where \; head_i= Attention(QW_{i}^{Q},KW_{i}^{K},VW_{i}^{V})\\\\ 其中\; W_{i}^Q \in \Bbb{R}^{d_{model}\times d_k} ,W_{i}^K \in \Bbb{R}^{d_{model}\times d_k} , W_{i}^V \in \Bbb{R}^{d_{model}\times d_v} ,W_{i}^O \in \Bbb{R}^{hd_v\times d_{model}}
