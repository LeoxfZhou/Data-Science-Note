...
2. **单网络运行 (Single CNN)**：在图像上运行单个卷积神经网络（主干网络为 DarkNet 架构）。
3. **后处理优化 (Post-processing)**：根据模型输出的置信度，对检测结果进行**非极大值抑制 (NMS, Non-Maximum Suppression)**，消除冗余重复框。
![[YOLO V1, V2, V3-20260626102735623.png]]
...
![[YOLO V1, V2, V3-20260626102902456.png]]
### 2. 网格预测内容
...
![[YOLO V1, V2, V3-20260626103030505.png|428]]
...
![[YOLO V1, V2, V3-20260626103219188.png]]
...
- **后 $20$ 个通道（$C$）**：该网格对应的 20 个类别的条件类别概率。
    ![[YOLO V1, V2, V3-20260626104046415.png|454]]
...
![[YOLO V1, V2, V3-20260626103632919.png]]
![[YOLO V1, V2, V3-20260626104007823.png]]
...
# YOLOv2：更准、更快、更强的演进之路 (YOLO9000: Better, Faster, Stronger)

YOLOv2（在结合大规模分类数据集时称为 YOLO9000）针对 YOLOv1 存在的**定位不准**和**召回率 (Recall)** 较低的致命缺陷进行了全面升级。论文从三个核心维度对模型进行了重新设计：

1. **更准 (Better)**：专注于提升检测精度，使得 **平均精度均值 (mAP, Mean Average Precision)** 从 YOLOv1 的 63.4% 大幅攀升至 **78.6%**，追平了当时的 Faster R-CNN 和 SSD。
...
- **做法**：在网络中每一个**卷积层 (Convolutional Layer)** 后面都无脑加入批归一化层，并移除原本的 Dropout。
- **作用**：有效缓解了**内部协变量偏移 (ICS, Internal Covariate Shift)** 问题，有助于规范化模型，起到**正则化 (Regularization)** 的效果，防止模型**过拟合 (Overfitting)**。
- **效果**：**mAP 提升了 2%**。
...
- **核心思想**：为了大幅提升对小尺度物体的检测效果，模型引入了**转移层 (Passthrough Layer / Space-to-Depth)**，其功能类似于 ResNet 的**残差连接 (Residual Connection)**，负责将浅层的丰富高分辨率特征强行融合进深层特征中。
- **实现机制**：
...
- **正样本 (Positive)**：与当前 Ground Truth 的 **交并比 (IoU)** 大于设定阈值，且在所有预设 Anchor 中 **IoU 得分最高** 的那个 Anchor。只有正样本才同时计算坐标损失、置信度损失和类别损失。
- **负样本 (Negative)**：与所有 Ground Truth 的 IoU 都**小于设定阈值**的 Anchor。被判定为背景，**仅计算置信度损失 (Confidence Loss)**。
- **忽略样本 (Ignored)**：与某个 Ground Truth 的 IoU 大于设定阈值，但**不是最高的那一个**。为了避免歧义，这部分 Anchor **不计入任何损失计算**（即不参与梯度更新）。
