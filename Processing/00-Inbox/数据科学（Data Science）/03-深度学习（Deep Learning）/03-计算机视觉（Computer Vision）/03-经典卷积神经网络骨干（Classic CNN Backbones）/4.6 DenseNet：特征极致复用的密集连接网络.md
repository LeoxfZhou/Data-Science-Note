dense block, 1 * 1降纬不损失信息
source:https://zhuanlan.zhihu.com/p/37189203
## 核心概述与荣誉
...
![[4.6 DenseNet：特征极致复用的密集连接网络-20260726174438319.png|478]]
传统网络与 ResNet 仅关注相邻或相近几层之间的连接，而 DenseNet 互相连接了所有的层。具体而言，**每一个层都会接受其前面所有层作为额外的输入**。对于一个包含 $L$ 层的网络，DenseNet 共包含 $\frac{L(L+1)}{2}$ 个连接。

### 2. 网络前向传播公式对比
![[4.6 DenseNet：特征极致复用的密集连接网络-20260726174454168.png]]
...
- **DenseNet 模型**：直接连接（Concat，c）前面所有层的特征图作为输入：
    ...
    _(注：$H_\ell$ 代表一个组合非线性转化函数，通常包含 BN、ReLU、Pooling 和 Conv 操作。)_

## 网络架构细节
![[4.6 DenseNet：特征极致复用的密集连接网络-20260726174410258.png|515]]
![[4.6 DenseNet：特征极致复用的密集连接网络-20260726174521889.png]]
...
![[4.6 DenseNet：特征极致复用的密集连接网络-20260726174540856.png|554]]
...
![[4.6 DenseNet：特征极致复用的密集连接网络-20260726174555339.png]]
### 3. Transition 层（过渡层）
...
1. **`_DenseLayer`**：负责 bottleneck 结构（1x1 卷积升/降维 + 3x3 卷积），并在 `forward` 中将输入 `x` 与新生成的特征 `new_features` 在 `dim=1`（通道维度）上进行 `torch.cat` 拼接。
