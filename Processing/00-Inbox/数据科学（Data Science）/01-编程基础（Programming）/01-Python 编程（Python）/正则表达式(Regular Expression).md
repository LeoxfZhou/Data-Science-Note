...
许多语言都支持利用正则表达式进行字符串操作，Python 也不例外，通过内置的 `re` 模块实现。
---
## 二、 字符匹配 (Character Matching)
- **匹配机制 (Matching Mechanism)**：对要匹配的字符串的元素挨个判断是否与“规则”匹配。
- **书写注意**：正则表达式是字符串，所以书写时通常不要随意加空格。
- **反斜杠问题 (Backslash Issue)**：因为正则表达式和 Python 字符串都使用反斜杠字符来转义，有时就需要使用双倍的反斜杠才能达到想要的效果，而这很麻烦，所以强烈建议大家在写正则表达式时，用**原始字符串 (Raw String)**。
...
### `*`
- 对它前面的正则表达式匹配 0 到任意次重复，尽量多的匹配（贪婪匹配 Greedy Match）。
...
### `+`
- 对它前面的正则表达式匹配 1 到任意次重复，尽量多的匹配（贪婪）。
...
### `?`
- 对它前面的正则表达式匹配 0 到 1 次，尽量多的匹配（贪婪）。
...
### `\` (转义与特殊序列 Escape & Special Sequence)
- 转义特殊字符。
...
### `\number`
- 匹配数字代表的分组里面的内容（每个括号是一个子组，子组从 1 开始编号），在 `[` 和 `]` 字符集内，任何数字转义都被看作是字符。
...
### `\A`
- 匹配字符串的开头，类似于 `^`，区别在于：`MULTILINE` 模式下，`\A` 不识别换行。
...
### `\b` 与 `\B`
- `\b`：匹配空字符串，但只在单词开始或结尾的位置，即匹配一个**单词边界 (Word Boundary)**。
...
- 当字符串的起始位置匹配成功，返回 `Match` 类的实例对象（该实例对象包含匹配相关的信息：起始和结束位置、匹配的子串等等）；如果起始位置没有匹配，则返回 `None`。
...
- `pos`：匹配的起始位置，默认为 0。
- `endpos`：匹配的结束位置，默认为字符串长度。
- 当整个字符串都匹配成功，返回 `Match` 类的实例对象（该实例对象包含匹配相关的信息：起始和结束位置、匹配的子串等等），否则返回 `None`。
...
---
## 四、 模块级别函数 (Module Level Functions)
如果你不想创建 `Pattern` 实例对象并调用其方法，也可以直接使用 `re` 模块提供的函数：
...
## 编译模式
编译标志可以修改正则表达式的一些匹配方式，它在 re 模块中有两个名  
称:全名 和 缩写。其中需要掌握的有:
### `re.I / re.IGNORECASE`
- 进行忽略大小写匹配
### `import re`
