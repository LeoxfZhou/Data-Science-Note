---
title: 03-网页解析：正则、XPath 与 Beautiful Soup（Web Parsing）
tags:
  - python/web-scraping/parsing
status: published
...
# 输出：每一行都是由当前 HTML 文档决定的元素列表；没有匹配项时输出 []。
```
...
print(soup.img)
print(soup.find('img'))
# 输出：依次输出当前文档中的第一个 title、div 和 img 标签；没有对应标签时输出 None。
```

#### 2-3 获取属性

```python
# 使用点语法或 find() 时，默认都只获取第一个匹配结果。
...
# 输出：第一行是原始 div 标签，后续为同一标签经过缩进和换行美化后的多行 HTML。
```
...
# 输出：每个选择器返回一个 Tag 对象列表；具体元素取决于 soup 中的 HTML，未匹配时为 []。
```
