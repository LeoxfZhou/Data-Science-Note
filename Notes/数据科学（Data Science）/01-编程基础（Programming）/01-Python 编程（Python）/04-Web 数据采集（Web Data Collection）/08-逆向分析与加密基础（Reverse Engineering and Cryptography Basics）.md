---
title: 08-逆向分析与加密基础（Reverse Engineering and Cryptography Basics）
tags:
  - python/web-scraping/reverse-engineering
status: published
...
```python
print(execjs.get().name)  # 输出取决于本机 JavaScript 运行时，例如 Node.js (V8)
...
# 可能出现：execjs._exceptions.ProgramError: Error: Malformed UTF-8 data
# 若返回数据两端带有额外的单引号或双引号，需要先去除外层引号再解码。
```
...
sha = sha256(b'salt')
sha.update(b'alex')
print(sha.hexdigest())  # 20b191e632d18e4b11695e9f060ee37aff117a94f01fd04a04d021c76a5f6c57
```
...
Traceback (most recent call last):
  File "<project>/example.py", line 33, in <module>
    bb = base64.b64decode(s)
  File "<python-install>/lib/base64.py", line 87, in b64decode
...
# 需要加密的数据必须是16的倍数
# 填充规则: 缺少数据量的个数 * 对应数值的单字节
pad_len = 16 - len(data_bs) % 16
data_bs += bytes([pad_len]) * pad_len
# 再对编码后的字节进行加密
bs = aes.encrypt(data_bs)
#用base64对结果进行编码
result = base64.b64encode(bs).decode()
print(result)  # 9noPO0fcQizMbPkXcVOTDg==
```
...
print(res)  # f67a0f3b47dc422ccc6cf9177153930e

# 也可以转换成16进制，跟上面一个效果一样
print(bs.hex())  # f67a0f3b47dc422ccc6cf9177153930e
```
...
print(base64_res)  # t4TYyzRnIkmVmI81n+cdsVQfprHN5AtG
```
...
print(mingwen)  # 艾尼在学爬虫
```

注：DES3、AES 与 DES 的代码流程都包含创建密码器、填充、加密/解密和编码转换，但密钥长度、分组大小、安全性与可选模式不同，不能直接互换参数。

注：DES 的标准密钥输入为 8 字节。JavaScript 库如何处理超长密钥取决于具体实现，不应假定所有库都会自动截取前 8 字节。
...
#rsa每次加密后的结果可能不一样
print(base_64)  # 每次输出的 Base64 密文通常不同，因为 PKCS#1 v1.5 填充包含随机字节
