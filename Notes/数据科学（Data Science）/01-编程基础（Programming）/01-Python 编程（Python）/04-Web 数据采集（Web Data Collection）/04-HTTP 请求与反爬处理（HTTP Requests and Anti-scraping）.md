---
title: 04-HTTP 请求与反爬处理（HTTP Requests and Anti-scraping）
tags:
  - python/web-scraping/http
status: published
...
# 获取登录后的数据
url = 'https://user.17k.com/ck/user/myInfo/USER_ID?bindInfo=1&appKey=YOUR_APP_KEY'
res = requests.get(url, headers=headers, cookies=cookies)
print(res.content.decode())
```
...
![[Attachments/Notes/数据科学（Data Science）/01-编程基础（Programming）/01-Python 编程（Python）/04-Web 数据采集（Web Data Collection）/04-HTTP 请求与反爬处理（HTTP Requests and Anti-scraping）/04-HTTP 请求与反爬处理（HTTP Requests and Anti-scraping）-20240130143846000.png|12306ssl错误]]
