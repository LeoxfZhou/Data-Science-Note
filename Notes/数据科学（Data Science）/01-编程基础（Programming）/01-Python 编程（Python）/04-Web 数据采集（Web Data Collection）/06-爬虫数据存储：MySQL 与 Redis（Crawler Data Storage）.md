---
title: 06-爬虫数据存储：MySQL 与 Redis（Crawler Data Storage）
tags:
  - python/web-scraping/storage
status: published
...
![[Attachments/Notes/数据科学（Data Science）/01-编程基础（Programming）/01-Python 编程（Python）/04-Web 数据采集（Web Data Collection）/06-爬虫数据存储：MySQL 与 Redis（Crawler Data Storage）/06-爬虫数据存储：MySQL 与 Redis（Crawler Data Storage）-20240130144316000.png|img]]
...
> [!warning] 原附件缺失（Missing Source Attachment）
> 原始资料引用了 `image-20230718225320480.png`，但该文件未随原稿提供。
> [!warning] 原附件缺失（Missing Source Attachment）
> 原始资料引用了 `image-20230718225334560.png`，但该文件未随原稿提供。
```mysql
## 步骤一：开启事务
    1，Set autocommit = 0;
...
> [!warning] 原附件缺失（Missing Source Attachment）
> 原始资料引用了 `image-20230718225816775.png`，但该文件未随原稿提供。
> [!warning] 原附件缺失（Missing Source Attachment）
> 原始资料引用了 `image-20230718225827213.png`，但该文件未随原稿提供。
##### 1.5.6 注意
...
![[Attachments/Notes/数据科学（Data Science）/01-编程基础（Programming）/01-Python 编程（Python）/04-Web 数据采集（Web Data Collection）/06-爬虫数据存储：MySQL 与 Redis（Crawler Data Storage）/06-爬虫数据存储：MySQL 与 Redis（Crawler Data Storage）-20230924175359617.png|image-20230924175359617]]
...
> [!warning] 原附件缺失（Missing Source Attachment）
> 原始资料引用了 `image-20230718231923557.png`，但该文件未随原稿提供。
```mysql
3 查看用户变量的值
	## （1）SELECT @用户变量名
```
...
# 连接MySQL数据库
db = pymysql.connect(host='127.0.0.1', user='root', password='YOUR_DB_PASSWORD', database='test',port=3306)
...
r = redis.Redis(host='127.0.0.1',port=6379,password='YOUR_DB_PASSWORD',db=0,decode_responses=True) #默认数据库为0 

r = redis.StrictRedis(host='127.0.0.1',port=6379,password='YOUR_DB_PASSWORD',decode_responses=True)
```
...
```python
pool = redis.ConnectionPool(host='127.0.0.1',port=6379,db=0,password='YOUR_DB_PASSWORD',decode_responses=True)
