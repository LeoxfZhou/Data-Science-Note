...
> [!example]- 类、实例与属性传递图示（Class, Instance, and Attribute Flow）
> ![[Attachments/Notes/数据科学（Data Science）/01-编程基础（Programming）/01-Python 编程（Python）/01-基础与标准库（Fundamentals and Standard Library）/05-面向对象编程（Object-Oriented Programming）/05-面向对象编程（Object-Oriented Programming）-1742542965063.png]]
...
在属性名或方法名前添加两个下划线会触发名称改写（Name Mangling）。外部使用原名称（例如 `obj.__age`）通常会触发 `AttributeError`，但改写后的名称仍可被访问，因此它用于减少子类意外覆盖，不是安全隔离。对外读写应优先通过方法或 `property` 提供稳定接口。
```Python
class Person:
    school = '深兰教育'
    __eat = 'rice' # 私有类属性
...
> [!example]- 基础继承关系图示（Basic Inheritance Diagram）
> ![[Attachments/Notes/数据科学（Data Science）/01-编程基础（Programming）/01-Python 编程（Python）/01-基础与标准库（Fundamentals and Standard Library）/05-面向对象编程（Object-Oriented Programming）/05-面向对象编程（Object-Oriented Programming）-1742543766695.png]]
