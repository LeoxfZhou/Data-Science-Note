---
title: "OpenCV 图像处理原理：颜色、滤波、形态学与特征（OpenCV Image Processing Principles）"
tags:
  - data-science/cv/opencv/principles
status: published
created: 2026-08-12
published_at: 2026-08-12
---
# OpenCV 图像处理原理：颜色、滤波、形态学与特征（OpenCV Image Processing Principles）
> [!tip] 大白话理解（Plain-language Intuition）
> 卷积/滤波（Convolution/Filtering）可以理解为拿一个小窗口在图像上滑动，每到一处就按模板权重汇总邻域像素。平滑核让邻居互相平均，因此能压噪但可能抹平边缘；梯度核比较相邻区域变化，因此能突出边缘但也会放大噪声。核的权重、尺寸、步长和边界填充共同决定“看多大范围、保留什么、丢掉什么”。
## 一、 OpenCV 概述与安装
### 1. OpenCV 简介
...
## 颜色空间补充与原图（Color-space Supplement and Source Figure）
颜色空间
- rgb：人眼的色彩空间
- opencv使用bgr
- hsv/hsb/hsl
...
hsl
![[02-OpenCV 图像处理原理（OpenCV Image Processing Principles）-20260410113833251.png]]
左:HSL,右边:HSV
YUV（视频）
Mat浅拷贝
