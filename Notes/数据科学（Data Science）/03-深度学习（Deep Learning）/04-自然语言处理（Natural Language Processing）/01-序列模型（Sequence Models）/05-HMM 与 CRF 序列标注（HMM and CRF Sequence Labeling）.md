---
title: "隐马尔可夫模型与条件随机场（Hidden Markov Model and Conditional Random Field）"
tags:
  - data-science/nlp
status: published
created: 2026-08-13
...
---
# 隐马尔可夫模型与条件随机场（Hidden Markov Model and Conditional Random Field）

### 1 HMM模型介绍
> [!tip] 大白话理解（Plain-language Intuition）
> HMM 假设看不见的状态按马尔可夫链变化，并由每个状态生成可观察结果。做序列标注时，就是根据整段观测推断最可能的隐藏标签序列。
#### 1.1 HMM模型的输入和输出
...
### 2 CRF模型介绍
> [!tip] 大白话理解（Plain-language Intuition）
> CRF 直接给整条标签序列打分，并把相邻标签之间的约束纳入全局归一化。它能避免逐位置分类产生不合法的标签跳转，但训练和解码需要动态规划。
#### 2.1 CRF模型的输入和输出
