---
title: "基于 NumPy 的三层 BP 神经网络（Three-layer BP Neural Network with NumPy）"
tags:
  - data-science/deep-learning/numpy-bp
status: published
created: 2026-08-12
published_at: 2026-08-12
---
# 基于 NumPy 的三层 BP 神经网络（Three-layer BP Neural Network with NumPy）
> [!tip] 大白话理解（Plain-language Intuition）
> 前向传播（Forward Propagation）像按当前参数完成一次答题，损失函数（Loss Function）负责算“错了多少”；反向传播（Backpropagation）用链式法则（Chain Rule）把总错误逐层拆成每个权重应承担的份额，优化器（Optimizer）再沿着让错误下降的方向小步修改参数。任何一层的矩阵形状、平均系数或激活函数导数写错，前面各层都会收到错误的更新信号。
## 第一部分：重构充填【死磕级全行注释代码】
