---
title: "FastAPI 核心开发参考（FastAPI Core Development Reference）"
tags:
  - data-science/deployment/fastapi
status: published
created: 2026-08-12
published_at: 2026-08-12
---
# FastAPI 核心开发参考（FastAPI Core Development Reference）
> [!tip] 大白话理解（Plain-language Intuition）
> 异步（Asynchronous）适合“等待网络、磁盘或数据库时先去接别的请求”，并不会让同步的 CPU/GPU 推理自动并行。若在 `async def` 中直接运行长时间阻塞推理，事件循环（Event Loop）就像只有一个接线员却被一通长电话占住；普通 `def` 可由 FastAPI 放到线程池执行，但 GPU 共享、批处理、背压和吞吐仍需单独设计。
## FastAPI 核心开发笔记
### 2 | 安装 (Installation)
